...
$post = $_POST;
// Here I am going to parse each post value into its corresponding array
// Post fields in the form "answerXXX" go to $answer[XXX]
// Post fields in the form "defXXX" go to $def[XXX]
// Post fields in the form "questionXXX" go to $userchoice[XXX]
// Post fields in the form "wordXXX" go to $word[XXX]
foreach($post as $key => $value) {
    if(substr($key, 0, 6) == 'answer') {
        ...
        @file_put_contents('quizAnswers.txt', $value."\t".gmdate('Y-m-d\TH:i:s\Z')."\n", FILE_APPEND);
    }
}
